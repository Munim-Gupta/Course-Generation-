...
def update_module_status(user_id, module_id):
    ...
    video_watched == 1 AND quiz_completed == 1
    """
    prog = get_or_create_progress(user_id, module_id)
    if prog:
        is_completed = 1 if (prog["video_watched"] == 1 and prog["quiz_completed"] == 1) else 0
        execute_db(
...
def mark_video_watched(module_id):
    ...
    execute_db(
        "UPDATE progress SET video_watched = 1 WHERE user_id = ? AND module_id = ?",
        (user_id, module_id)
    )
    update_module_status(user_id, module_id)

    flash("Video marked as watched! Great progress.", "success")
